...
#ifndef BITCOIN_NETI2PD_H
#define BITCOIN_NETI2PD_H
...
CI2pdNode* FindNode(const CDestination& ip);
CI2pdNode* ConnectNode(CI2PAddress addrConnect, const char* pszDest = NULL, bool obfuScationMaster = false);
bool OpenNetworkConnection(const CI2PAddress& addrConnect, CSemaphoreGrant* grantOutbound = NULL, const char* strDest = NULL, bool fOneShot = false);
void MapPort(bool fUseUPnP);
unsigned short GetListenPort();
bool BindListenPort(const CDestination& bindAddr, std::string& strError, bool fWhitelisted = false);
...
struct CNodeSignals {
    ...
    boost::signals2::signal<bool(CI2pdNode*)> ProcessMessages;
    boost::signals2::signal<bool(CI2pdNode*, bool)> SendMessages;
    boost::signals2::signal<bool()> AddressRefreshBroadcast;
    boost::signals2::signal<void(NodeId, const CI2pdNode*)> InitializeNode;
...
bool AddLocal(const CDestination& addr, int nScore = LOCAL_NONE);
bool AddLocal(const CI2pUrl& addr, int nScore = LOCAL_NONE);
bool RemoveLocal(const CDestination& addr);
bool SeenLocal(const CDestination& addr);
bool IsLocal(const CDestination& addr);
bool GetLocal(CDestination& addr, const CI2pUrl* paddrPeer = NULL);
bool IsReachable(enum Network net);
bool IsReachable(const CI2pUrl& addr);
void SetReachable(enum Network net, bool fFlag = true);
CI2PAddress GetLocalAddress(const CI2pUrl* paddrPeer = NULL);
...
extern std::vector<CI2pdNode*> vNodes;
...
extern std::map<CI2pUrl, LocalServiceInfo> mapLocalHost;
...
typedef std::map<CI2pSubNet, CBanEntry> banmap_t;
...
{
public:
    // I2PDK: instead of sockets we're using tunnels, tunnel could be client, server or null tunnel.
    // I2PPureClientTunnel, I2PPureServerTuneel, I2PNullTunnel (invalid) 
    std::shared_ptr<i2p::client::I2PService> i2pTunnel;
    ...
    bool fOneShot;
    bool fClient;
    // I2PDK: server (true) or client (false)
    bool fInbound;
    bool fNetworkNode;
    ...
    // this should be private (not really, it's used within functions)
//private:
    //i2p::client::SendCallback _sendCallback;
    //i2p::client::SendCallback _sendMoreCallback;
    //i2p::client::SendMoreCallback _sendMoreCallback;
    //i2p::client::ContinueToReceiveCallback _continueToReceiveCallback;

    // actually just save the connection and get rid of the send callbacks, we don't need that
    // and connection will probably not changed for the lifetime of the tunnel (both are destroyed and recreated) or
    // even if it changes we'll get another created callback to set it here (just make sure we release resources if any
    // related to the callbacks)
    i2p::client::ErrorSendCallback _errorCallback;
    std::shared_ptr<i2p::client::I2PPureTunnelConnection> _connection;
...
protected:
    ...
    static std::vector<CI2pSubNet> vWhitelistedRange;
...
public:
    ...
    // flood relay
    std::vector<CI2PAddress> vAddrToSend;
    mruset<CI2PAddress> setAddrKnown;
    bool fGetAddr;
    ...
    CI2pdNode(std::shared_ptr<i2p::client::I2PService> tunnelIn, CI2PAddress addrIn, std::string addrNameIn = "", bool fInboundIn = false);
    ~CI2pdNode();
...
private:
    ...
    CI2pdNode(const CI2pdNode&);
    void operator=(const CI2pdNode&);
...
public:
    ...
    void AddAddressKnown(const CI2PAddress& addr)
    {
        setAddrKnown.insert(addr);
    }

    void PushAddress(const CI2PAddress& addr)
    {
    ...
    static bool IsBanned(CI2pUrl ip);
    static bool IsBanned(CI2pSubNet subnet);
    static void Ban(const CI2pUrl &ip, const BanReason &banReason, int64_t bantimeoffset = 0, bool sinceUnixEpoch = false);
    static void Ban(const CI2pSubNet &subNet, const BanReason &banReason, int64_t bantimeoffset = 0, bool sinceUnixEpoch = false);
    static bool Unban(const CI2pUrl &ip);
    static bool Unban(const CI2pSubNet &ip);
    ...
    static bool IsWhitelistedRange(const CI2pUrl& ip);
    static void AddWhitelistedRange(const CI2pSubNet& subnet);
