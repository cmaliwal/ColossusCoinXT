    ...
    if (ui->radioListMode->isChecked()) {
        ...
        for (int i = 0; i < ui->treeWidget->topLevelItemCount(); i++) {
            ...
            if (model->isLockedCoin(uint256(item->text(COLUMN_TXHASH).toStdString()), item->text(COLUMN_VOUT_INDEX).toUInt())) {
                ...
                item->setIcon(COLUMN_CHECKBOX, QIcon(GUIUtil::getIconPath("lock_closed")));
            }
            updateLabelLocked();
        }
        ui->treeWidget->setEnabled(true);
...
void CoinControlDialog::lockCoin()
{
    ...
    contextMenuItem->setIcon(COLUMN_CHECKBOX, QIcon(GUIUtil::getIconPath("lock_closed")));
    updateLabelLocked();
}
        ...
        for(const COutput& out: coins.second) {
            ...
            if (fMultiSigUTXO) {
                ...
                if (!fMultisigEnabled) {
                    ...
                    itemOutput->setIcon(COLUMN_CHECKBOX, QIcon(GUIUtil::getIconPath("lock_closed")));
                }
            } else {
