	...
	namespace util
	{
      ...
      return true;
		...
		bool Daemon_Singleton::start()
		{
			...
			LogPrint(eLogInfo, "Daemon: starting custom client/server stream.");
			//i2p::client::context.Start();
			...
			// HttpServer
			// ShowLeaseSetDestination (std::stringstream& s, std::shared_ptr<const i2p::client::LeaseSetDestination> dest
			// std::shared_ptr<const i2p::client::LeaseSetDestination> dest
			// dest->GetNumRemoteLeaseSets()
			// dest->GetIdentity ()->ToBase64 ()
			// auto pool = dest->GetTunnelPool ();
			// pool->GetOutboundTunnels ()
			// 
			...
			//}
			// it // outboundtunnel->GetEndpointIdentHash ()
