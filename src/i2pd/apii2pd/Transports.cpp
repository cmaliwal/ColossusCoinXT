...
namespace transport
{
    ...
    void Transports::CreateLeaseSetTest()
    {
        ...
        for (auto& it : i2p::client::context.GetClientPureTunnels())
        {
            ...
            std::string dest = it.second->GetDestination();
            ...
            if (!leaseSet)
            {
                LogPrint(eLogInfo, "CreateLeaseSetTest: no lease set, recreate...");
                    ...
                    [&newDataReceived, &leaseSet, &newDataReceivedMutex, &dest](std::shared_ptr<i2p::data::LeaseSet> ls)
                {
                    if (!ls) {
                        ...
                        LogPrint(eLogError, "CreateLeaseSetTest.RequestDestination: LeaseSet for address ", dest, " not found");
                        return;
                    } else {
                        LogPrint(eLogInfo, "CreateLeaseSetTest: LeaseSet set!");
                        leaseSet = ls;
                ...
                {
                    LogPrint(eLogError, "CreateLeaseSetTest: Subscription LeaseSet request timeout expired");
            ...
            if (!leaseSet1) {
                /* still no leaseset found */
                LogPrint(eLogError, "CreateLeaseSetTest: LeaseSet for address ", dest, " not found");
                return;
            }
    ...
    void Transports::ClientServerTest()
    {
        ...
        for (auto& it : i2p::client::context.GetClientPureTunnels())
        {
            ...
            std::static_pointer_cast<i2p::client::TCPIPAcceptor>(it.second)->InvokeHandleAccept();
            //static_cast<TCPIPAcceptor*>(it.second)->InvokeHandleAccept();
            //it.second->InvokeHandleAccept();

            continue;

            auto stream = localDest->CreateStream(leaseSet);
            LogPrint(eLogInfo, "ClientServerTest: stream acquired, sending...");
            std::string request = "test something to send, test data...";
            ...
            while (!end)
            {
                    ...
                    [&](const boost::system::error_code& ecode, std::size_t bytes_transferred)
                {
                    ...
                    i2p::client::SUBSCRIPTION_REQUEST_TIMEOUT);
                std::unique_lock<std::mutex> l(newDataReceivedMutex);
                // wait 1 more second
                if (newDataReceived.wait_for(l, std::chrono::seconds(i2p::client::SUBSCRIPTION_REQUEST_TIMEOUT + 1)) == std::cv_status::timeout)
                {
                    LogPrint(eLogError, "ClientServerTest: subscriptions request timeout expired");
                    numAttempts++;
            ...
            /* parse response */
            LogPrint(eLogDebug, "ClientServerTest: response: ", response);
