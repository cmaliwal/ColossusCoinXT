...
namespace client
{
    ...
    void ClientContext::Stop ()
    {
        ...
        for (auto& it : m_ServerPureTunnels)
        {
            LogPrint(eLogInfo, "Clients: stopping I2P pure server tunnel");
            it.second->Stop();
        }
        m_ServerPureTunnels.clear();
        ...
        for (auto& section: pt)
        {
            ...
            try
            {
                ...
                if (type == I2P_TUNNELS_SECTION_TYPE_CLIENT
                        || type == I2P_TUNNELS_SECTION_TYPE_CLIENT_PURE
                        ...
                        || type == I2P_TUNNELS_SECTION_TYPE_UDPCLIENT)
                {
                    ...
                    if (type == I2P_TUNNELS_SECTION_TYPE_UDPCLIENT) {
                        ...
                        else if (type == I2P_TUNNELS_SECTION_TYPE_CLIENT_PURE)
                        {
                            // client w/o sockets just streaming
                            LogPrint(eLogInfo, "tunnels.read: name: ", name, ", dest:", dest, ", address:", address, ", port:", port, ", local:", localDestination->GetIdentHash().ToBase32());

                            // tcp client
                            auto tun = std::make_shared<I2PPureClientTunnel>(name, dest, address, port, localDestination, destinationPort);
                        ...
                        } else
                        {
                            ...
                            // tcp client
                            auto tun1 = std::make_shared<I2PPureClientTunnel>(name, dest, address, port, localDestination, destinationPort);
                        ...
                        else
                        {
                            ...
                            LogPrint (eLogInfo, "Clients: I2P client tunnel for endpoint ", clientEndpoint, " already exists");
                                 ...
                                 || type == I2P_TUNNELS_SECTION_TYPE_SERVER_PURE
                                 ...
                                 || type == I2P_TUNNELS_SECTION_TYPE_UDPSERVER)
                {
                    ...
                    else if (type == I2P_TUNNELS_SECTION_TYPE_SERVER_PURE)
                    {
                        std::shared_ptr<I2PPureServerTunnel>  serverTunnel;
                        serverTunnel = std::make_shared<I2PPureServerTunnel>(name, host, port, localDestination, inPort, gzip);
    ...
    bool ClientContext::InsertStartServerTunnel(i2p::data::IdentHash hash, int port, std::shared_ptr<I2PPureServerTunnel> tunnel)
    {
        //std::pair<std::map<std::pair<i2p::data::IdentHash, int>, std::shared_ptr<I2PPureServerTunnel>>::iterator, bool> ret = 
        auto ins = m_ServerPureTunnels.insert(std::make_pair(std::make_pair(hash, port), tunnel));
        ...
        else
        {
            ...
            ins.first->second->isUpdated = true;
            LogPrint(eLogInfo, "Clients: I2P server tunnel for destination/port ", m_AddressBook.ToAddress(hash), "/", port, " already exists");
            return false;
        }
    }
