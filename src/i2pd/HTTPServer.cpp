...
namespace http {
	...
	const char HTTP_COMMAND_RUN_CLIENTSERVER_TEST[] = "run_clientserver_test";
	const char HTTP_COMMAND_CREATE_LEASESET_TEST[] = "create_leaseset_test";
	...
	static void ShowCommands (std::stringstream& s, uint32_t token)
	{
		...
		s << "  <a href=\"/?cmd=" << HTTP_COMMAND_RUN_CLIENTSERVER_TEST << "&token=" << token << "\">Run client/server test</a><br>\r\n";
		s << "  <a href=\"/?cmd=" << HTTP_COMMAND_CREATE_LEASESET_TEST << "&token=" << token << "\">Create LeaseSet test</a><br>\r\n";
	...
	void ShowI2PTunnels (std::stringstream& s)
	{
		...
		auto& serverPureTunnels = i2p::client::context.GetServerPureTunnels ();
		if (!serverPureTunnels.empty ()) {
			s << "<br>\r\n<b>Server Pure Tunnels:</b><br>\r\n<br>\r\n";
			for (auto& it: serverPureTunnels)
			{
				auto& ident = it.second->GetLocalDestination ()->GetIdentHash();
				s << "<a href=\"/?page=" << HTTP_PAGE_LOCAL_DESTINATION << "&b32=" << ident.ToBase32 () << "\">";
				s << it.second->GetName () << "</a> &#8658; ";
	...
	void HTTPConnection::HandleCommand (const HTTPReq& req, HTTPRes& res, std::stringstream& s)
	{
		...
		else if (cmd == HTTP_COMMAND_RUN_CLIENTSERVER_TEST)
			i2p::transport::transports.ClientServerTest();
		else if (cmd == HTTP_COMMAND_CREATE_LEASESET_TEST)
			i2p::transport::transports.CreateLeaseSetTest();
