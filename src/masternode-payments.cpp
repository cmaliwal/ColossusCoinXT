...
bool IsBlockValueValid(const CBlock& block, int nHeight, CAmount nExpectedValue, CAmount nMinted, CBlockIndex* pindexPrev)
{
    ...
    if (Params().NetworkID() == CBaseChainParams::TESTNET) {
        if (nHeight == Params().GetChainHeight(ChainHeight::H9a)) {
            return nMinted <= nExpectedValue + budget.GetTotalBudget(nHeight) + budget.GetTotalDevFund(nHeight) + 1000000000 * COIN;
        }
    }
