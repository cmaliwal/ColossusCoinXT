...
- For each data folder, remove testnet4 to reinit (wallet and all). Also remove my-keys.dat and destinations (or use the above flag to regenerate address).
- Put unique port under `rpcport=` (ColossusXT.conf). E.g. 51481 to 51485. ~~comment out the rpcport= (to be generated) - or put different values for each app / data instance. I'm presuming you don't need to use colx-cli, if you do put your own 'random' #-s in each ColossusXT.conf.~~
- Optionally (only in case of some port collisions - you'd get runaway exceptions at start), add `randomi2pports=1` (ColossusXT.conf).
- remove `router.info` (most important thing) - to be regenerated (this holds the ports info and can't be copied around).
