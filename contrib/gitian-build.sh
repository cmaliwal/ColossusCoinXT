...
then
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit colx=${COMMIT} --url colx=${url} ../colx/contrib/gitian-descriptors/gitian-aarch64.yml
	    ./bin/gsign -p $signProg --signer $SIGNER --release ${VERSION}-aarch64 --destination ../gitian.sigs/ ../colx/contrib/gitian-descriptors/gitian-aarch64.yml
	    mv build/out/colx-*.tar.gz build/out/src/colx-*.tar.gz ../colx-binaries/${VERSION}
	popd
...
then
	...
	echo ""
	./bin/gverify -v -d ../gitian.sigs/ -r ${VERSION}-aarch64 ../colx/contrib/gitian-descriptors/gitian-aarch64.yml
	# Signed Windows
	echo ""
